...
pkt_push_ip(lbuf_t *b, ip_addr_t *src, ip_addr_t *dst, int proto)
{
    ...
    if (ip_addr_afi(src) != ip_addr_afi(dst)) {
        LMLOG(LDBG_1, "src %s and dst %s IP have different AFI! Discarding!",
        ...
        ip_addr_t *dip)
{
    ...
    if (pkt_push_udp(b, sp, dp) == NULL) {
        LMLOG(LDBG_1, "Failed to push UDP header! Discarding");
        return(BAD);
    }
    ...
    if (pkt_push_ip(b, sip, dip, IPPROTO_UDP) == NULL) {
        LMLOG(LDBG_1, "Failed to push IP header! Discarding");
        return(BAD);
    }
    ...
    if (udpsum == -1) {
        LMLOG(LDBG_1, "Failed UDP checksum! Discarding");
        return (BAD);
    }
    udpsum(uh) = udpsum;
    return(GOOD);
...
pkt_parse_5_tuple(lbuf_t *b, packet_tuple_t *tuple)
{
    ...
    case 6:
        ...
        break;
    default:
        LMLOG(LDBG_2, "pkt_parse_5_tuple: Not an IP packet!");
        return (BAD);
    }
        ...
        lisp_addr_t *addr_dest, int port_from,int port_dest, int *encap_pkt_len)
{
    ...
    if ((lisp_addr_ip_afi(addr_from) != AF_INET)
        ...
        LMLOG(LDBG_2, "add_ip_udp_header: Unknown AFI %d",
