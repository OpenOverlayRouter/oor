...
new_lcl_locator_extended_info(int *out_socket)
{
    ...
    if (!(lcl_loc_ext_inf = malloc(sizeof(lcl_locator_extended_info)))) {
        lmlog(LWRN, "lcl_locator_extended_info: Unable to allocate memory for "
                "rmt_locator_extended_info: %s", strerror(errno));
        return(NULL);
    }
...
locator_t *new_local_locator(lisp_addr_t *locator_addr, uint8_t *state,
        ...
        int *out_socket)
{
    ...
    if ((locator = malloc(sizeof(locator_t))) == NULL) {
        lmlog(LWRN, "new_local_locator: Unable to allocate memory for "
...
new_rmt_locator_extended_info()
{
    ...
    if (!(rmt_loc_ext_inf = malloc(sizeof(rmt_locator_extended_info)))) {
        lmlog(LWRN, "new_rmt_locator_extended_info: Unable to allocate memory "
                "for rmt_locator_extended_info: %s", strerror(errno));
        return(NULL);
    }
...
locator_t *
new_static_rmt_locator(lisp_addr_t *rloc_addr, uint8_t state, uint8_t priority,
        uint8_t weight, uint8_t mpriority, uint8_t mweight)
{
    locator_t *locator = NULL;

    if ((locator = malloc(sizeof(locator_t))) == NULL) {
        lmlog(LWRN, "new_static_rmt_locator: Unable to allocate memory for "
                "lispd_locator_elt: %s", strerror(errno));
        return (NULL);
    }
...
int locator_list_add(locators_list_t **list, locator_t *loc) {
...
rtr_locator_list_new()
{
    ...
    rtr_list = malloc(sizeof(rtr_locators_list));
    if (rtr_list == NULL) {
        lmlog(LWRN, "Unable to allocate memory for rtr_locators_list: %s",
                strerror(errno));
        return (ERR_MALLOC);
    }
    return(rtr_list);
