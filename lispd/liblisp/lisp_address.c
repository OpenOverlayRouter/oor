...
lisp_addr_get_iana_afi(lisp_addr_t *laddr)
{
    ...
    default:
        LMLOG(LDBG_2, "lisp_addr_get_iana_afi: unknown AFI (%d)",
                lisp_addr_lafi(laddr));
        return (BAD);
    }
}
...
lisp_addr_size_to_write(lisp_addr_t *laddr)
{
    ...
    default:
        LMLOG(LDBG_3, "lisp_addr_get_size_in_pkt: not defined for afi %d",
                lisp_addr_lafi(laddr));
        break;
    }
    return (0);
...
lisp_addr_get_plen(lisp_addr_t *laddr)
{
    ...
    default:
//        lispd_log_msg(LDBG_2, "lisp_addr_get_plen: not defined for afi %d",
...
lisp_addr_to_char(lisp_addr_t *addr)
{
    ...
    case LM_AFI_NO_ADDR:
        return ("_NO_ADDR_");
    default:
        LMLOG(LDBG_3, "lisp_addr_to_char: Trying to convert"
...
lisp_addr_ip_to_ippref(lisp_addr_t *laddr)
{
    if (lisp_addr_lafi(laddr) != LM_AFI_IP
        ...
        LMLOG(LDBG_3, "lisp_addr_ip_to_ippref: called, but addr has afi (%d)",
                lisp_addr_lafi(laddr));
        return;
    }
    set_lafi_(laddr, LM_AFI_IPPREF);
...
lisp_addr_ip_afi(lisp_addr_t *addr)
{
    ...
    default:
        LMLOG(LDBG_1, "lisp_addr_ip_afi: not supported for afi %d",
                get_lafi_(addr));
        return (0);
    }
}
...
lisp_addr_ip_get_addr(lisp_addr_t *laddr)
{
    if (get_lafi_(laddr) != LM_AFI_IP && get_lafi_(laddr) != LM_AFI_IPPREF) {
        LMLOG(LDBG_3, "lisp_addr_ip_get_addr: called, but addr has afi (%d)",
                get_lafi_(laddr));
        return (NULL);
    }
    ...
    case LM_AFI_NO_ADDR:
    case LM_AFI_LCAF:
        LMLOG(LDBG_3, "lisp_addr_ip_get_addr: AFI (%s) not of IP type",
                get_lafi_(laddr));
        break;
    default:
        LMLOG(LDBG_3, "lisp_addr_ip_get_addr: AFI (%s) not supported",
                get_lafi_(laddr));
    }
    return (NULL);
}
...
lisp_addr_ip_get_plen(lisp_addr_t *laddr)
{
    ...
    case LM_AFI_IPPREF:
        return (ip_prefix_get_plen(get_ippref_(laddr)));
    default:
        LMLOG(LDBG_3, "lisp_addr_ip_get_plen: called with AFI not IP or IPPREF");
    }
...
lisp_addr_ip_set_afi(lisp_addr_t *laddr, int afi)
{
    ...
    case LM_AFI_IPPREF:
        ...
        break;
    default:
        LMLOG(LDBG_3, "lisp_addr_ip_set_afi: called with LM AFI %d",
                get_lafi_(laddr));
        break;
    }
}
...
lisp_addr_set_plen(lisp_addr_t *laddr, uint8_t plen)
{
    ...
    case LM_AFI_IPPREF:
        ...
        break;
    default:
        LMLOG(LDBG_2, "lisp_addr_set_plen: not supported for afi %d",
                lisp_addr_lafi(laddr));
        break;
    }
}
...
lisp_addr_copy(lisp_addr_t *dst, lisp_addr_t *src)
{
    ...
    case LM_AFI_NO_ADDR:
        LMLOG(LDBG_3, "lisp_addr_copy:  No address element copied");
        break;
    case LM_AFI_IP:
    ...
    case LM_AFI_LCAF:
        ...
        break;
    default:
        LMLOG(LDBG_2, "lisp_addr_copy:  Unknown AFI type %d in EID",
                lisp_addr_lafi(dst));
        break;
    }
}
...
lisp_addr_copy_to(void *dst, lisp_addr_t *src)
{
    ...
    case LM_AFI_LCAF:
        ...
        break;
    default:
        LMLOG(LDBG_3, "lisp_addr_copy_to:  Unknown AFI type %d in EID",
                lisp_addr_lafi(src));
        break;
    }
    return (0);
...
lisp_addr_write(void *offset, lisp_addr_t *laddr)
{
    ...
    case LM_AFI_NO_ADDR:
        ...
        return (sizeof(uint16_t));
    default:
        LMLOG(LDBG_3, "lisp_addr_write_to_pkt: Unkown afi %d",
                lisp_addr_lafi(laddr));
        break;
    }
    return (0);
...
lisp_addr_parse(uint8_t *offset, lisp_addr_t *laddr)
{
    ...
    if (!laddr) {
        LMLOG(LDBG_3, "lisp_addr_parse: Called with unallocated address!");
        return (0);
    }
...
//        laddr->lcaf = lcaf_addr_new();
    ...
    case LISP_AFI_NO_ADDR:
        ...
        break;
    default:
        LMLOG(LDBG_2, "lisp_addr_read_from_pkt:  Unknown AFI type %d in EID",
                afi);
        break;
    }
...
lisp_addr_cmp(lisp_addr_t *addr1, lisp_addr_t *addr2)
{
    int cmp;
    if (!addr1 || !addr2) {
        LMLOG(LDBG_3,"lisp_addr_cmp: One of the compared addresses is NULL");
        return (-1);
    }
    if (lisp_addr_lafi(addr1) != lisp_addr_lafi(addr2)) {
        LMLOG(LDBG_3,"lisp_addr_cmp: Addresses with different lafi: %d - %d",
...
lisp_addr_cmp_afi(lisp_addr_t *addr1, lisp_addr_t *addr2)
{
    ...
    case LM_AFI_IP:
        ...
        break;
    case LM_AFI_IPPREF:
        LMLOG(LDBG_1,"locator_list_cmp_afi: No locators of type prefix");
        return (-2);
    case LM_AFI_LCAF:
...
lisp_addr_get_ip_addr(lisp_addr_t *addr)
{
    ...
    case LM_AFI_IP:
    	return (addr);
    case LM_AFI_IPPREF:
    	LMLOG(LWRN, "lisp_addr_get_ip_addr: Not applicable to prefixes");
        return (NULL);
    case LM_AFI_LCAF:
