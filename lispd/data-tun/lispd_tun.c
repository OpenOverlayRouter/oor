        ...
        lisp_addr_t *tun_v6_addr)
{
    ...
    if (router_mode == FALSE)
    {
        ...
        if (tun_v4_addr != NULL){
            if (tun_add_eid_to_iface(tun_v4_addr) != GOOD){
                return (BAD);
            }
        ...
        if (tun_v6_addr != NULL){
            if (tun_add_eid_to_iface(tun_v6_addr) != GOOD){
                return (BAD);
            }
        ...
        if (tun_v4_addr != NULL){
            add_route(AF_INET,iface_index,NULL,NULL,NULL,RULE_TO_LISP_TABLE_PRIORITY,LISP_TABLE);
        }
        if (tun_v6_addr != NULL){
            add_route(AF_INET6,iface_index,NULL,NULL,NULL,RULE_TO_LISP_TABLE_PRIORITY,LISP_TABLE);
        }
    }
    return (GOOD);
}
