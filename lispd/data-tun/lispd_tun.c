...
//    memcopy_lisp_addr((void *)((char *)rta + sizeof(struct rtattr)),&eid_address);
    ...
    LMLOG(LISP_LOG_DEBUG_1, "added %s EID to TUN interface.",lisp_addr_to_char(&eid_address));
    close(sockfd);
    return(GOOD);
}
...
int set_tun_default_route_v4()
{
    ...
    lisp_addr_ip_from_char("0.0.0.0",&gw);
    ...
    lisp_addr_ip_from_char("0.0.0.0",&dest);
    ...
    lisp_addr_ip_from_char("128.0.0.0",&dest);
...
int set_tun_default_route_v6()
{
    ...
    lisp_addr_ip_from_char("8000::",&dest);
