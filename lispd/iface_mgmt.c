...
process_address_change(iface_t *iface, lisp_addr_t *new_addr)
{
    ...
    if (iface_addr == NULL){
        LMLOG(LDBG_2,"precess_address_change: LISPmob not configured to use %s address for the interface %s",
...
process_nl_new_link(struct nlmsghdr *nlh)
{
    ...
    if (iface->status == new_status){
        LMLOG(LDBG_2,"process_nl_new_link: The detected change of status"
                " doesn't affect");
        return;
    }
    /* raise event to data plane */
...
process_nl_new_unicast_route(struct rtmsg *rtm, int rt_length)
{
    ...
    if (lisp_addr_ip_afi(&gateway) != LM_AFI_NO_ADDR &&
        ...
        LMLOG(LDBG_1, "process_nl_new_unicast_route gateway %s  in iface %s",
...
//    lisp_addr_t grpaddr = {.lafi = LM_AFI_IP};
    ...
    LMLOG(LDBG_1, "process_nl_new_multicast_route: Not yet implented, ignored!");
