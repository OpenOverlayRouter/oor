...
lispd_jib_t *lispd_new_jib() {
    ...
    jib =  lispd_generic_list_new(NULL, free_lispd_locators_list);
    return(jib);
}
...
lispd_generic_list_t *jib_get_orlist(lispd_jib_t *jib) {
    ...
    orlist = lispd_generic_list_new(NULL, free_lispd_locators_list);
    ...
    generic_list_for_each_entry(it,jib){
        ...
        lispd_generic_list_add(jibentry->locators->locator, orlist);
    }
...
//    jibentry->timer = smth;
    ...
    lispd_generic_list_add(jibentry, jib);
}
...
inline uint32_t jib_size(lispd_jib_t *jib) {
    return(lispd_generic_list_size(jib));
}
