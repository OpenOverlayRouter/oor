...
void jni_init(JNIEnv *env, jobject object)
{
    ...
    class_android_jni = (*jni_env)->FindClass(jni_env,"org/lispmob/noroot/LISPmob_JNI");
    if (class_android_jni == NULL){
        LMLOG(LINF,"JNI Error: Class not found: org.lispmob.noroot.LISPmob_JNI");
        return;
    }
...
void lispd_jni_protect_socket(int socket)
{
    ...
    if (meth_protect == NULL){
        LMLOG(LINF,"JNI Error: Method not found: org.lispmob.noroot.LISPmob_JNI:jni_protect_socket");
        return;
    }
