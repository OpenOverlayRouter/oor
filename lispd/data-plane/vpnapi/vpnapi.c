...
int vpnapi_configure_data_plane(lisp_dev_type_e dev_type, ...);
void vpnapi_uninit_data_plane();
int vpnapi_add_datap_iface_addr(iface_t *iface, int afi);
int vpnapi_add_eid_prefix(lisp_dev_type_e dev_type, lisp_addr_t *eid_prefix);
int vpnapi_remove_eid_prefix(lisp_dev_type_e dev_type, lisp_addr_t *eid_prefix);
...
int
vpnapi_configure_data_plane(lisp_dev_type_e dev_type, ...)
{
    ...
    if (default_rloc_afi != AF_INET6){
        ...
        lispd_jni_protect_socket(data->ipv4_data_socket);
    }else {
    ...
    if (default_rloc_afi != AF_INET){
        ...
        lispd_jni_protect_socket(data->ipv6_data_socket);
    }else {
...
int
vpnapi_add_eid_prefix(lisp_dev_type_e dev_type, lisp_addr_t *eid_prefix)
{
    return (GOOD);
}

int
vpnapi_remove_eid_prefix(lisp_dev_type_e dev_type, lisp_addr_t *eid_prefix)
{
    return (GOOD);
}
        ...
        lisp_addr_t *dst_pref, lisp_addr_t *gateway)
{
    if (lisp_addr_ip_afi(gateway) != LM_AFI_NO_ADDR
        ...
        LMLOG(LDBG_1,  "vpnapi_updated_route: Process new gateway "
...
vpnapi_process_new_gateway(iface_t *iface,lisp_addr_t *gateway)
{
    ...
    if (iface->status != UP){
        LMLOG(LDBG_1,"vpnapi_process_new_gateway: Probably the interface %s is UP "
...
vpnapi_updated_addr(iface_t *iface,lisp_addr_t *old_addr,lisp_addr_t *new_addr)
{
    ...
    if (lisp_addr_cmp(old_addr, new_addr) == 0) {
        LMLOG(LDBG_2, "vpnapi_updated_addr: The change of address detected "
...
vpnapi_update_link(iface_t *iface, int old_iface_index, int new_iface_index, int status)
{
    ...
    if (old_iface_index != new_iface_index){
        ...
        LMLOG(LDBG_2, "vpnapi_update_link: The new index of the interface "
...
vpnapi_reset_socket(int fd, int afi)
{
    ...
    switch (afi){
    case AF_INET:
        LMLOG(LDBG_2,"reset_socket: Reset IPv4 data socket");
        ...
        break;
    case AF_INET6:
        LMLOG(LDBG_2,"reset_socket: Reset IPv6 data socket");
