...
int tun_configure_data_plane(lisp_dev_type_e dev_type, ...);
void tun_uninit_data_plane();
int tun_add_datap_iface_addr(iface_t *iface,int afi);
int tun_add_eid_prefix(lisp_dev_type_e dev_type, lisp_addr_t *eid_prefix);
int tun_remove_eid_prefix(lisp_dev_type_e dev_type, lisp_addr_t *eid_prefix);
 ...
 */
int
tun_configure_data_plane(lisp_dev_type_e dev_type, ...)
{
...
int
tun_add_eid_prefix(lisp_dev_type_e dev_type, lisp_addr_t *eid_prefix){
    switch(dev_type){
    case xTR_MODE:
...
int
tun_remove_eid_prefix(lisp_dev_type_e dev_type, lisp_addr_t *eid_prefix){
    switch(dev_type){
    case xTR_MODE:
...
tun_del_eid_from_iface(lisp_addr_t *addr)
{
    ...
    if (sockfd < 0) {
        LMLOG(LERR, "tun_del_eid_from_iface: Failed to connect to netlink socket");
        return(BAD);
    }
    ...
    case AF_INET6:
        ...
        break;
    default:
        LMLOG(LERR, "tun_del_eid_from_iface: Address no IP address %s",
                lisp_addr_to_char(addr));
        return(BAD);
    }
...
//    memcopy_lisp_addr((void *)((char *)rta + sizeof(struct rtattr)),&eid_address);
    ...
    if (retval < 0) {
        LMLOG(LERR, "tun_del_eid_from_iface: send() failed %s", strerror(errno));
        close(sockfd);
        return(BAD);
    }

    LMLOG(LDBG_1, "Removed %s EID from TUN interface.",lisp_addr_to_char(addr));
    close(sockfd);
    return(GOOD);
}
        ...
        lisp_addr_t *dst_pref, lisp_addr_t *gateway)
{
    ...
    if (lisp_addr_ip_afi(gateway) != LM_AFI_NO_ADDR
        ...
        if (ip_addr_is_link_local(lisp_addr_ip(gateway)) == TRUE) {
            LMLOG(LDBG_3,"tun_update_route: the extractet address "
        ...
        LMLOG(LDBG_1,  "tun_update_route: Process new gateway "
...
tun_updated_addr(iface_t *iface, lisp_addr_t *old_addr, lisp_addr_t *new_addr)
{
    ...
    if (lisp_addr_cmp(old_addr, new_addr) == 0) {
        LMLOG(LDBG_2, "tun_updated_addr: The change of address detected "
    ...
    if (old_addr_lafi == LM_AFI_NO_ADDR) {
        LMLOG(LDBG_2, "tun_updated_addr: Generating sockets for the initialized interface "
...
tun_process_new_gateway(iface_t *iface,lisp_addr_t *gateway)
{
    ...
    if (*gw_addr == NULL) { // The default gateway of this interface is not deffined yet
        *gw_addr = lisp_addr_new();
        ...
        LMLOG(LDBG_3,"tun_process_new_gateway: the gatweay address has not changed: %s. Discard message.",
                            lisp_addr_to_char(gateway));
        return;
    }else{
        lisp_addr_copy(*gw_addr,gateway);
...
tun_get_default_output_address(int afi)
{
    ...
    case AF_INET6:
        ...
        break;
    default:
        LMLOG(LDBG_2, "tun_get_default_output_address: AFI %s not valid", afi);
        return(NULL);
    }
...
tun_get_default_output_socket(int afi)
{
    ...
    case AF_INET6:
        ...
        break;
    default:
        LMLOG(LDBG_2, "tun_get_default_output_socket: AFI %s not valid", afi);
        break;
    }
