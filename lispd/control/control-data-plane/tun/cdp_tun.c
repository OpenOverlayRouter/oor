...
int
tun_control_dp_init(lisp_ctrl_t *ctrl, ...);
void
tun_control_dp_uninit (lisp_ctrl_t *ctrl);
int
tun_control_dp_add_iface_addr(lisp_ctrl_t *ctrl,iface_t *iface, int afi);
int
tun_control_dp_recv_msg(sock_t *sl);
int
tun_control_dp_send_msg(lisp_ctrl_t *ctrl, lbuf_t *buff, uconn_t *udp_conn);
lisp_addr_t * tun_control_dp_get_default_addr(lisp_ctrl_t *ctrl, int afi);
int
tun_control_dp_updated_route(lisp_ctrl_t *ctrl, int command, iface_t *iface,
        lisp_addr_t *src_pref, lisp_addr_t *dst_pref, lisp_addr_t *gw);
int
tun_control_dp_updated_addr(lisp_ctrl_t *ctrl, iface_t *iface,lisp_addr_t *old_addr,
        lisp_addr_t *new_addr);
int
tun_control_dp_update_link(lisp_ctrl_t *ctrl, iface_t *iface, int old_iface_index,
...
tun_control_dp_send_msg(lisp_ctrl_t *ctrl, lbuf_t *buff, uconn_t *udp_conn)
{
    ...
    if (lisp_addr_lafi(&udp_conn->ra) != LM_AFI_IP) {
        LMLOG(LDBG_2, "tun_control_dp_send_msg: Destination address %s of UDP connection is not a IP. "
    ...
    if (ip_addr_afi(src_addr) != ip_addr_afi(dst_addr)) {
        LMLOG(LDBG_2, "tun_control_dp_send_msg: src %s and dst %s of UDP connection have "
    ...
    if (ret != GOOD) {
        LMLOG(LDBG_1, "Failed to send contrl message from RLOC: %s -> %s",
                lisp_addr_to_char(&udp_conn->la), lisp_addr_to_char(&udp_conn->ra));
        return(BAD);
    } else {
        LMLOG(LDBG_1, "Sent control message IP: %s -> %s UDP: %d -> %d",
                lisp_addr_to_char(&udp_conn->la), lisp_addr_to_char(&udp_conn->ra),
...
lisp_addr_t *
tun_control_dp_get_default_addr(lisp_ctrl_t *ctrl, int afi)
{
    ...
    case AF_INET6:
        ...
        break;
    default:
        LMLOG(LDBG_2,"tun_control_dp_get_default_addr: Unsupported afi: %d",afi);
        break;
    }
        ...
        uconn_t *udp_conn)
{
    ...
    if (lisp_addr_is_no_addr(&udp_conn->la)) {
        ...
        if (!ctrl_addr) {
            LMLOG(LERR, "tun_control_dp_get_output_ctrl_sock: No %s control address found, send aborted!",
        ...
        if (iface != NULL) {
            ...
            LMLOG(LDBG_2, "tun_control_dp_get_output_ctrl_sock: No interface found with local address %s. Using default one!",
                    lisp_addr_to_char(&udp_conn->la));
            ctrl_addr = tun_control_dp_get_default_ctrl_address(data, dst_afi);
            if (!ctrl_addr) {
                LMLOG(LERR, "tun_control_dp_get_output_ctrl_sock: No control address found, send aborted!");
                return(ERR_SOCKET);
            }
            /* Use as local address the default control address */
            lisp_addr_copy(&udp_conn->la, ctrl_addr);
