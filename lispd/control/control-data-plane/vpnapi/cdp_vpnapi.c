...
int
vpnapi_control_dp_init(lisp_ctrl_t *ctrl,...);
void
vpnapi_control_dp_uninit (lisp_ctrl_t *ctrl);
int
vpnapi_control_dp_add_iface_addr(lisp_ctrl_t *ctrl,iface_t *iface, int afi);
int
vpnapi_control_dp_recv_msg(sock_t *sl);
int
vpnapi_control_dp_send_msg(lisp_ctrl_t *ctrl, lbuf_t *buff, uconn_t *udp_conn);
lisp_addr_t *
vpnapi_control_dp_get_default_addr(lisp_ctrl_t *ctrl, int afi);
int
vpnapi_control_dp_updated_route(lisp_ctrl_t *ctrl, int command, iface_t *iface,
        lisp_addr_t *src_pref,lisp_addr_t *dst_pref, lisp_addr_t *gw);
void
vpnapi_control_dp_process_new_gateway(lisp_ctrl_t *ctrl, iface_t *iface,
        lisp_addr_t *gateway);
int
vpnapi_control_dp_updated_addr(lisp_ctrl_t *ctrl, iface_t *iface,
        lisp_addr_t *old_addr,lisp_addr_t *new_addr);
int
vpnapi_control_dp_update_link(lisp_ctrl_t *ctrl, iface_t *iface,
...
vpnapi_control_dp_init(lisp_ctrl_t *ctrl, ...)
{
    ...
    if (default_rloc_afi != AF_INET6) {
        ...
        lispd_jni_protect_socket(data->ipv4_ctrl_socket);
    }else {
    ...
    if (default_rloc_afi != AF_INET) {
        ...
        lispd_jni_protect_socket(data->ipv6_ctrl_socket);
    }else {
...
vpnapi_control_dp_send_msg(lisp_ctrl_t *ctrl, lbuf_t *buff, uconn_t *udp_conn)
{
    ...
    if (lisp_addr_lafi(&udp_conn->ra) != LM_AFI_IP) {
        LMLOG(LDBG_2, "vpnapi_control_dp_send_msg: Destination address %s of UDP connection is not a IP. "
    ...
    if (!lisp_addr_is_no_addr(&udp_conn->la) &&  (ip_addr_afi(src_addr) != ip_addr_afi(dst_addr))) {
        LMLOG(LDBG_2, "vpnapi_control_dp_send_msg: src %s and dst %s of UDP connection have "
        ...
        lisp_addr_t *src_pref, lisp_addr_t *dst_pref, lisp_addr_t *gateway)
{
    if (lisp_addr_ip_afi(gateway) != LM_AFI_NO_ADDR
        ...
        if (ip_addr_is_link_local(lisp_addr_ip(gateway)) == TRUE) {
            LMLOG(LDBG_3,"vpnapi_updated_route: the extractet address "
        ...
        lisp_addr_t *old_addr,lisp_addr_t *new_addr)
{
    ...
    if (lisp_addr_cmp(old_addr, new_addr) == 0) {
        LMLOG(LDBG_2, "vpnapi_control_dp_updated_addr: The change of address detected "
...
vpnapi_control_dp_reset_socket(vpnapi_ctr_dplane_data_t * data, int fd, int afi)
{
    ...
    switch (afi){
    case AF_INET:
        LMLOG(LDBG_2,"reset_socket: Reset IPv4 control socket\n");
        new_fd = open_control_input_socket(AF_INET);
        if (new_fd == ERR_SOCKET){
            LMLOG(LDBG_2,"vpnapi_reset_socket: Error recreating the socket");
            return (BAD);
        }
        data->ipv4_ctrl_socket = new_fd;
        break;
    case AF_INET6:
        LMLOG(LDBG_2,"reset_socket: Reset IPv6 control socket\n");
    ...
    lispd_jni_protect_socket(new_fd);
