        ...
        lisp_addr_t     local_rloc)
{
    ...
    if (err != GOOD){
        LMLOG(LDBG_1,"process_info_reply_msg: Couldn't process Info Reply message");
        return (BAD);
    }
    ...
    if ( ntohs(*lcaf_afi) != LISP_AFI_LCAF){
        LMLOG(LDBG_1,"process_info_reply_msg: Malformed packet");
        return (BAD);
    }
    ...
    if (err == BAD) {
        LMLOG(LDBG_2, "process_info_reply_msg: Error extracting packet data");
        return (BAD);
    }
    ...
    if (is_loggable(LDBG_2)){
        ...
        LMLOG(LDBG_2, "Info-Reply message data->"
    ...
    if (nonce_check(nat_ir_nonce,nonce) == GOOD ){
        LMLOG(LDBG_2, "Info-Reply: Correct nonce field checking ");
        free(nat_ir_nonce);
        nat_ir_nonce = NULL;
    }else{
        LMLOG(LDBG_1, "Info-Reply: Error checking nonce field. No Info Request generated with nonce: %s",
                nonce_to_char (nonce));
        return (BAD);
    }
    ...
    if(BAD == check_auth_field(key_id, map_servers->key, (void *) packet, pckt_len, auth_data_pos)){
        LMLOG(LDBG_2, "Info-Reply: Error checking auth data field");
        return(BAD);
    }else{
        LMLOG(LDBG_2, "Info-Reply: Correct auth data field checking");
    }
    ...
    case 0:
        is_behind_nat = FALSE;
        LMLOG(LDBG_2, "NAT Traversal: MN is not behind NAT");
        break;
    case 1:
    case 2:
        is_behind_nat = TRUE;
        LMLOG(LDBG_2, "NAT Traversal: MN is behind NAT");
        break;
    case -1:
        is_behind_nat = UNKNOWN;
        LMLOG(LDBG_2, "NAT Traversal: Unknown state");
        break;
    }
    ...
    if (is_behind_nat == TRUE){
        ...
        if (rtr_locators_list == NULL){
            LMLOG(LWRN, "process_info_reply_msg: The interface with IP address %s is behind NAT"
        ...
        if (mapping == NULL){
            LMLOG(LDBG_2, "process_info_reply_msg: Info Reply is not for any local EID");
            return (BAD);
        }
        locator = mapping_get_loct_with_addr(mapping, &local_rloc);
        if (locator == NULL){
            LMLOG(LDBG_2, "process_info_reply_msg: Info Reply received in the wrong locator");
            return (BAD);
        }
    ...
    if (is_behind_nat == TRUE){
        ...
        LMLOG(LDBG_1, "Reprogrammed info request in %d minutes",ttl);
    }else{
