...
lisp_addr_t *lisp_addr_new();
lisp_addr_t *lisp_addr_new_lafi(uint8_t lafi);
...
int lisp_addr_cmp(lisp_addr_t *addr1, lisp_addr_t *addr2);
int lisp_addr_cmp_afi(lisp_addr_t *addr1, lisp_addr_t *addr2);
uint32_t lisp_addr_size_to_write(lisp_addr_t *laddr);
char *lisp_addr_to_char(lisp_addr_t *addr);

void lisp_addr_set_lafi(lisp_addr_t *addr, lm_afi_t afi);
void lisp_addr_set_plen(lisp_addr_t *laddr, uint8_t plen);
void lisp_addr_set_ip(lisp_addr_t *addr, ip_addr_t *ip);
...
void lisp_addr_ip_set_afi(lisp_addr_t *laddr, int afi);
void lisp_addr_set_ip_afi(lisp_addr_t *la, int afi);
void lisp_addr_ip_to_ippref(lisp_addr_t *laddr);
void lisp_addr_ip_init(lisp_addr_t *addr, void *data, int afi);


int lisp_addr_is_lcaf(lisp_addr_t *laddr);
void lisp_addr_lcaf_set_addr(lisp_addr_t *laddr, void *addr);
void *lisp_addr_lcaf_addr(lisp_addr_t *laddr);
lcaf_type_e lisp_addr_lcaf_type(lisp_addr_t *laddr);
void lisp_addr_lcaf_set_type(lisp_addr_t *laddr, int type);

int lisp_addr_init_from_ip(lisp_addr_t *, ip_addr_t *);
int lisp_addr_init_from_ippref(lisp_addr_t *, ip_addr_t *, uint8_t);
int lisp_addr_init_from_lcaf(lisp_addr_t *, lcaf_addr_t *);

uint16_t lisp_addr_iana_afi_to_lm_afi(uint16_t afi);
uint16_t lisp_addr_get_iana_afi(lisp_addr_t *laddr);
uint16_t lisp_addr_get_plen(lisp_addr_t *laddr);

int lisp_addr_is_mc(lisp_addr_t *addr);
