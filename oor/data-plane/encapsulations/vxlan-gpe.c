...
void *
vxlan_gpe_data_push_hdr(lbuf_t *b, uint32_t vni, vxlan_gpe_nprot_t np)
{
    vxlan_gpe_hdr_t *vhdr;
    vhdr = lbuf_push_uninit(b, sizeof(vxlan_gpe_hdr_t));
    vxlan_gpe_data_hdr_init(vhdr, vni,np);
    return(vhdr);
}
...
vxlan_gpe_data_encap(lbuf_t *b, int lp, int rp, lisp_addr_t *la, lisp_addr_t *ra)
{
    ...
    vxlan_gpe_data_push_hdr(b, 329, np);
